...
const BulkOperations = ({ selectedBills, onComplete }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      showNotification(
        `${selectedBills.length} bills deleted successfully!`,
        "success",
      );
      onComplete();
    } catch (error) {
  ...
  const handleBulkUpdate = async () => {
    ...
    try {
      ...
      showNotification(
        `${selectedBills.length} bills updated successfully!`,
        "success",
      );
      onComplete();
    } catch (error) {
      ...
      <div className="bg-[#161b22] p-6 rounded-xl border border-[#30363d]">
        <div className="text-center py-8">
          ...
          <p className="text-[#8b949e] text-sm mt-1">
            Select bills to perform bulk operations
          </p>
        </div>
      </div>
    );
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setUpdateData({ ...updateData, vendor: e.target.value })
                }
                className="w-full p-3 bg-[#0d1117] border border-[#30363d] rounded-lg text-[#f0f6fc] focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-red-300 text-sm">
                    This will permanently delete {selectedBills.length} bills
                    and all their items.
                  </p>
                </div>
              </div>
